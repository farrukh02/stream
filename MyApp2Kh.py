from stringprep import in_table_c21
import streamlit as st
import pandas as pd

st.title('Барои синфи 9. Тести "Ба ман писанд аст"')
st.subheader('Дар зарфи 5 дақиқа аз санҷиш гузашта, соҳибӣ касби мувофиқ шавед')

chelovekPriroda=0
chelovekTechnika=0
chelovekChelovak=0
chelovekZnak=0
chelovekKhudObraz=0

choice = []
int1 = st.radio(
     "Фарз кардем, ки баъди таълими мувофиқ Шумо ҳама гуна  корро иҷро карда метавонед. Агар ба Шумо интихоби аз ду имконият якеро пешниҳод кунанд, кадомашро меписандед?",
     ('Нигоҳубини ҳайвонот', 'Идора кардани мошин ва дигар таҷҳизоти техникӣ (таъмир ва нигоҳубин)'))
if int1 == 'Нигоҳубини ҳайвонот':
    chelovekPriroda+=1
if int1 == 'Идора кардани мошин ва дигар таҷҳизоти техникӣ (таъмир ва нигоҳубин)':
    chelovekTechnika+=1

int2 = st.radio(
     "",
     ('Кӯмак ба беморон', 'Тартиб додани ҷадвал, нақша ва барномаҳо барои техникаи компютерӣ'))
if int2 == 'Кӯмак ба беморон':
    chelovekChelovak+=1
elif int2 == 'Тартиб додани ҷадвал, нақша ва барномаҳо барои техникаи компютерӣ':
    chelovekZnak+=1


int3 = st.radio(
     "",
     ('Назорат кардани сифати мусаввараи китобҳо, плакатҳо, открыткаҳои бадеӣ, картаҳои граммофон', 'Назорат кардани ҳолат ва сабзиши растаниҳо'))
if int3 == 'Назорат кардани сифати мусаввараи китобҳо, плакатҳо, открыткаҳои бадеӣ, картаҳои граммофон':
    chelovekKhudObraz+=1
else:
    chelovekPriroda+=1

int4 = st.radio(
     "",
     ('Коркарди масолеҳ (чӯб, матоъ, пластмасса ва ғайраҳо)', 'Муҳокимаи китобҳои бадеӣ (ё худ намоишномаҳо, консертҳо)'))
if int4 == 'Коркарди масолеҳ (чӯб, матоъ, пластмасса ва ғайраҳо)':
    chelovekTechnika+=1
else:
    chelovekChelovak+=1

int5 = st.radio(
     "",
     ('Муҳокима кардани асарҳо, мақолаҳои илмии оммафаҳм', 'Ба истеъмолкунандагон расондани маҳсулот, фурӯш ва таблиғоти он'))
if int5 == 'Муҳокима кардани асарҳо, мақолаҳои илмии оммафаҳм':
    chelovekZnak+=1
else:
    chelovekKhudObraz+=1


int6 = st.radio(
     "",
     ('Парвариши ҳайвоноти ҷавон (ин ё он ҷинс)', 'Машқ додани рафиқон ё кӯдакон барои иҷрои амалиёт (меҳнатӣ, таълимӣ, варзишӣ)'))
if int6 == 'Парвариши ҳайвоноти ҷавон (ин ё он ҷинс)':
    chelovekPriroda+=1
else:
    chelovekChelovak+=1


int7 = st.radio(
     "",
     ('Нусхабардории расму тасвирҳо (ё ҷур кардани асбобҳои мусиқӣ)', 'Идора кардани  ягон воситаи боркашони борбардор ё (нақлиёт) трактор, крани борбардор ва ғайраҳо'))
if int7 == 'Нусхабардории расму тасвирҳо (ё ҷур кардани асбобҳои мусиқӣ)':
    chelovekKhudObraz+=1
else:
    chelovekTechnika+=1

int8 = st.radio(
     "",
     ('Ба одамон фаҳмондани маълумотҳои ба онҳо зарур ', 'Оро додани намоишгоҳҳо, витринаҳо ё ширкат варзидан дар тайёр кардани намоишномаҳо, барномаҳо, барномаҳои рақсу суруд)'))
if int8 == 'Ба одамон фаҳмондани маълумотҳои ба онҳо зарур ':
    chelovekChelovak+=1
else:
    chelovekKhudObraz+=1



int9 = st.radio(
     "",
     ('Таъмири ашё, маҳсулот (либос, техника), манзилгоҳ', 'Ҷустуҷӯ ва ислоҳи хато дар матнҳо, ҷадвалҳо, тасвирҳо'))
if int9 == 'Таъмири ашё, маҳсулот (либос, техника), манзилгоҳ':
    chelovekTechnika+=1
else:
    chelovekZnak+=1


int10 = st.radio(
     "",
     ('Табобати ҳайвонот', 'Иҷрои ҳисобу китоб'))
if int10 == 'Табобати ҳайвонот':
    chelovekPriroda+=1
else:
    chelovekZnak+=1

int11= st.radio(
     "",
     ('Парвариш кардани намудҳои нави растаниҳо', 'Ба нақша гирифтан ва омода намудани навъҳои маҳсулоти саноатӣ (мошинаҳо, либос, хона, молҳои истеъмолӣ ва ғайраҳо)'))
if int11 == 'Парвариш кардани намудҳои нави растаниҳо':
    chelovekPriroda+=1
else:
    chelovekTechnika+=1


int12 = st.radio(
     "",
     ('Ҳаллу фасли баҳс, ҷанҷоли байни одамон, бовар кунондан, фаҳмонидан, танбеҳ додан, ҳавасманд кардан', 'Сарфаҳм рафтан ба нақшаҳо, тарҳҳо, ҷадвалҳо'))
if int12 == 'Ҳаллу фасли баҳс, ҷанҷоли байни одамон, бовар кунондан, фаҳмонидан, танбеҳ додан, ҳавасманд кардан':
    chelovekChelovak+=1
else:
    chelovekZnak+=1


int13 = st.radio(
     "",
     ('Мушоҳида намудан  ва омӯхтани фаъолияти маҳфилҳои   ҳаваскорони санъат.', 'Мушоҳида кардан ва омӯхтани ҳаёти микробҳо'))
if int13 == 'Мушоҳида намудан  ва омӯхтани фаъолияти маҳфилҳои   ҳаваскорони санъат.':
    chelovekKhudObraz+=1
else:
    chelovekPriroda+=1



int14 = st.radio(
     "",
     ('Идора кардан ва омӯхтани  дастгоҳҳои тиббӣ.', 'Расонидани ёрии тиббӣ ба одамон ҳангоми ҷарроҳат ёфтан, лат хӯрдан, сӯхтани ягон узви бадан ва ғ.'))
if int14 == 'Идора кардан ва омӯхтани  дастгоҳҳои тиббӣ.':
    chelovekTechnika+=1
else:
    chelovekChelovak+=1

int15 = st.radio(
     "",
     ('Ҳисоботи муфассал навиштан доир  ба зуҳурот, воқеоте, ки мавриди мушоҳида қарор доранд, объектҳои санҷишӣ ва ғайра', 'Иншо, тасвири бадеӣ намудани воқеот (аз рӯи мушоҳида, хаёл).'))
if int15 == 'Ҳисоботи муфассал навиштан доир  ба зуҳурот, воқеоте, ки мавриди мушоҳида қарор доранд, объектҳои санҷишӣ ва ғайра':
    chelovekZnak+=1
else:
    chelovekKhudObraz+=1

int16 = st.radio(
     "",
     ('Дар беморхона санҷишҳои лабораторӣ гузарондан.', 'Қабул, муоинаи беморон , бо онхо ҳамсуҳбат шудан, табобат таъин кардан.'))
if int16 == 'Дар беморхона санҷишҳои лабораторӣ гузарондан.':
    chelovekPriroda+=1
else:
    chelovekChelovak+=1


int17 = st.radio(
     "",
     ('Ранга кардани девори биноҳо ва сатҳи маснуот..', 'Иҷро кардани мантажи бино, ҷамъоварии мошинҳо.'))
if int17 == 'Ранга кардани девори биноҳо ва сатҳи маснуот..':
    chelovekKhudObraz+=1
else:
    chelovekTechnika+=1

int18 = st.radio(
     "",
     ('Ташкили сайёҳатҳои фарҳангии ҳамсолон ва  хурдсолон (ба теарт, осорхона), экскурсияҳо, роҳпаймоии туристӣ ва  ғ.', 'Дар саҳна бози кардан, иштирок дар консертҳо.'))
if int18 == 'Ташкили сайёҳатҳои фарҳангии ҳамсолон ва  хурдсолон (ба теарт, осорхона), экскурсияҳо, роҳпаймоии туристӣ ва  ғ.':
    chelovekChelovak+=1
else:
    chelovekKhudObraz+=1

int19 = st.radio(
     "",
     ('Аз руи нақша тайёркунии детал, маҳсулот (мошин, либос), сохтани биноҳо.', 'Машғул шудан ба нақшакашӣ, нусхабардории нақшаҳо,харитаҳо.'))
if int19 == 'Аз руи нақша тайёркунии детал, маҳсулот (мошин, либос), сохтани биноҳо.':
    chelovekTechnika+=1
else:
    chelovekZnak+=1


int20 = st.radio(
     "",
     ('Мубориза бурдан бар зидди касалиҳои растанӣ, ҳашароти зараррасони ҷангал, боғ.', 'Кор дар мошинҳои клавиатурадор (компютер).)'))
if int20 == 'Мубориза бурдан бар зидди касалиҳои растанӣ, ҳашароти зараррасони ҷангал, боғ.':
    chelovekPriroda+=1
else:
    chelovekZnak+=1


max_value = max(chelovekPriroda, chelovekTechnika, chelovekChelovak,chelovekZnak, chelovekKhudObraz)
secondMax = sorted(list([chelovekPriroda, chelovekTechnika, chelovekChelovak, chelovekZnak, chelovekKhudObraz]))[-2]

if max_value == chelovekPriroda:
    choice = 'Дар ҷои аввал ба Шумо мувофиқ аст: Инсон-табиат, касбҳои зерин ба шумо мувофиқанд: Чорводор, Боғбон, Деҳқон, Дарахтпарвар, Агроном, Ветеринар, Геолог, Биолог, Химик'
if max_value == chelovekTechnika:
    choice = 'Дар ҷои аввал ба Шумо мувофиқ аст: Инсон-техник, касбхои зерин ба шумо мувофиканд: Монтажчӣ, Дуредгар, Челонгар, Токар, Дӯзандагӣ, Гилкор, Электромонтёр, Инженер, Конструктор, Технолог, Кайҳоннавард'
if max_value == chelovekChelovak:
    choice = 'Дар ҷои аввал ба Шумо мувофиқ аст: Инсон-инсон, касбҳои зерин ба шумо мувофиқанд: Мураббии кӯдакистон, Ҳамшираи шафқат, Корманди милиса, Фурӯшанда, Корманди иҷтимоӣ, Пешхизмат, Менеҷер, Равоншинос, Ёвари котиб, Омӯзгор, Ҳуқуқшинос, Муфаттиш'
if max_value == chelovekZnak:
    choice = 'Дар ҷои аввал ба Шумо мувофиқ аст: Инсон-аломатҳои системавӣ, касбҳои зерин ба шумо мувофиқанд: Хазинадор, Оператори компютер, Ҳарфчин, Оператори телефон, Оператори алоқа, Стенограф, Муҳосиб, Сейсмолог, Тарҷумон, Барномасоз, Иқтисодчӣ'
if max_value == chelovekKhudObraz:
    choice = 'Дар ҷои дуюм ба Шумо мувофиқ аст: Инсон-Образи бадеӣ, касбҳои зерин ба шумо мувофиқанд: Буранда, Сартарош, Ошпаз, Дӯзандагӣ, Суратгир, Кандакор, Дизайнер, Меъмор, Мусиқачӣ, Хореограф, Рассом, Қайкалтарош, Ороишгар'
    
if secondMax == chelovekPriroda:
    choice2 = 'Дар ҷои дуюм ба Шумо мувофиқ аст: Инсон-табиат, касбҳои зерин ба шумо мувофиқанд: Чорводор, Боғбон, Деҳқон, Дарахтпарвар, Агроном, Ветеринар, Геолог, Биолог, Химик'
if secondMax == chelovekTechnika:
    choice2 = 'Дар ҷои дуюм ба Шумо мувофиқ аст: Инсон-техник, касбхои зерин ба шумо мувофиканд: Монтажчӣ, Дуредгар, Челонгар, Токар, Дӯзандагӣ, Гилкор, Электромонтёр, Инженер, Конструктор, Технолог, Кайҳоннавард'
if secondMax == chelovekChelovak:
    choice2 = 'Дар ҷои дуюм ба Шумо мувофиқ аст: Инсон-инсон, касбҳои зерин ба шумо мувофиқанд: Мураббии кӯдакистон, Ҳамшираи шафқат, Корманди милиса, Фурӯшанда, Корманди иҷтимоӣ, Пешхизмат, Менеҷер, Равоншинос, Ёвари котиб, Омӯзгор, Ҳуқуқшинос, Муфаттиш'
if secondMax == chelovekZnak:
    choice2 = 'Дар ҷои дуюм ба Шумо мувофиқ аст: Инсон-аломатҳои системавӣ, касбҳои зерин ба шумо мувофиқанд: Хазинадор, Оператори компютер, Ҳарфчин, Оператори телефон, Оператори алоқа, Стенограф, Муҳосиб, Сейсмолог, Тарҷумон, Барномасоз, Иқтисодчӣ'
if secondMax == chelovekKhudObraz:
    choice2 = 'Дар ҷои дуюм ба Шумо мувофиқ аст: Инсон-Образи бадеӣ, касбҳои зерин ба шумо мувофиқанд: Буранда, Сартарош, Ошпаз, Дӯзандагӣ, Суратгир, Кандакор, Дизайнер, Меъмор, Мусиқачӣ, Хореограф, Рассом, Қайкалтарош, Ороишгар'

if st.button('Натиҷа'):
     st.write(choice)
     st.write(choice2)
